...
import numpy as np
#To make the code reproducible, I set a random seed
random.seed(729)
PRINT_ERRORS=0
...
def testBayes(sentencesTest, dataName, pWordPos, pWordNeg, pWord,pPos):
    ...
    print("STEP2 Accuracy:", Accuracy)
    
    print("STEP2 Precision_Pos:", Precision_Pos)
    print("STEP2 Recall_Pos:", Recall_Pos)
    print("STEP2 F_measure_Pos:", F_measure_Pos)
    
    print("STEP2 Precision_Neg:", Precision_Neg)
    print("STEP2 Recall_Neg:", Recall_Neg)
    print("STEP2 F_measure_Neg:", F_measure_Neg)
    print()
...
def mostUseful(pWordPos, pWordNeg, pWord, n):
    ...
    print ("\nPOSITIVE:")
    print (tail)   
#Calculate how many 'most useful' words are in the sentiment dictionary    
    neg_in_dic_num = 0
    neg_not_in_dic_num = 0
    pos_in_dic_num = 0
    pos_not_in_dic_num = 0
    head1 = head
    tail1 = tail
    for word in head1:
        if word in sentimentDictionary:
    ...
    for word in tail1:
        if word in sentimentDictionary:
            ...
            word = "False"   
            pos_not_in_dic_num += 1   
    print("The number of words in the sentiment dictionary is",
          neg_in_dic_num + pos_in_dic_num)
    print("The number of words not in the sentiment dictionary is",
          neg_not_in_dic_num + pos_not_in_dic_num)

#---------------------------New Function for STEP5-3 ----------------------------------
#main idea: When a negation word and an intensifier are detected before an emotional word
#(of 2 words and 1 word respectively), the emotional score of this word is multiplied by
# minus 0.5 on the original basis. 
#When a negation word is detected before an emotional word, the emotional score of this 
#word is multiplied by minus 1 on the original basis.
#When an intensifier word is detected before an emotional word, the emotional score of this 
#word is multiplied by 2 on the original basis. 
def testDictionarynew(sentencesTest, dataName, sentimentDictionary, threshold):
    ...
    for sentence, sentiment in sentencesTest.items():
        ...
        for word in Words:
            if word in sentimentDictionary:
                if np.any(np.in1d(Words[current - 2 : current], negation_words)) and \
                    np.any(np.in1d(Words[current - 1], intensifiers)):
                    score+=(sentimentDictionary[word]*(-0.5))
                    current += 1
                elif np.any(np.in1d(Words[current - 2 : current], negation_words)):
                    score+=(sentimentDictionary[word]*(-1))
                    current += 1
                elif np.any(np.in1d(Words[current - 1], intensifiers)):
    ...
    print("STEP5-3 Accuracy:", Accuracy)
    
    print("STEP5-3 Precision_Pos:", Precision_Pos)
    print("STEP5-3 Recall_Pos:", Recall_Pos)
    print("STEP5-3 F_measure_Pos:", F_measure_Pos)
    
    print("STEP5-3 Precision_Neg:", Precision_Neg)
    print("STEP5-3 Recall_Neg:", Recall_Neg)
    print("STEP5-3 F_measure_Neg:", F_measure_Neg)
    print()
#---------- Main Script --------------------------
